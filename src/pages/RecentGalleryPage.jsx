...
export default function RecentGalleryPage({ setCurrentPage }) {
  const { recentProjects, categories, loading } = useContext(DesignContext);
  ...
  if (loading) {
    return (
      <div style={{ 
        ...
        <p style={{ color: "var(--navy)", fontSize: "1.1rem", fontWeight: "600" }}>Loading completed projects...</p>
        <style>{`
          @keyframes spin {
  ...
  const filteredProjects = filter === "all" 
    ? recentProjects 
    : recentProjects.filter(p => p.category === filter);
      ...
      <div className="container" style={{ maxWidth: "1100px" }}>
        ...
        <div style={{
          ...
          {[
            ...
            ...categories.map(cat => ({ 
              id: cat.id, 
              name: cat.name.replace(" Decorations", "s").replace(" Setup", "s") 
            }))
          ].map(btn => (
            <button
              key={btn.id}
        ...
        <div style={{
          ...
          {filteredProjects.map((proj) => {
            const categoryName = categories.find(c => c.id === proj.category)?.name || proj.category;
            return (
              <div 
                key={proj.id}
